...
How to use a computer's built-in TPM 2.0 chip to generate an SSH key and use
...
nixpkgs). Pinned below to the exact commit for **v0.9.0** (see comments in the
install commands) and explicitly noted in Notes.
...
- **Supply-chain check** before trusting the tool with signing keys: no
  ...
  install to the exact commit that maps to v0.9.0 and annotated the commands,
