...
- **Drove least-privilege access** across the platform — replaced a broad "read-all-products" Vault policy with per-team scoped policies via a staged, multi-wave rollout, backed by a custom transitive secret scanner to catch usage hidden in third-party actions and reusable workflows.
